* INTERNSHIP
  ...
  ** TODO [13/19]
   - [ ] UML Documents
     - [X] Sequence diagram
     - [ ] Others?
   - [ ] Transform the container in a gen_fsm machine
   - [ ] How are people writing ETS table for easy reading?
   - [ ] Central debug table to all actors (like ?DLOG but for history)
   - [ ] Dumb supervisors
     ...
     - [ ] Mode 4: round-robin
     - [ ] Mode 5: manual mode
     - [X] Mode 6: product driven (quality first on first product)
     - [ ] Mode 7: team-mode
   - [ ] Prettier CVS data
     - [X] Add data on RFID
   ...
   - [X] New actor for item erasing (mostly stats and table-handling)
   - [X] A scanner actor 
   - [ ] Easier debug
  ...
  **  LATER [1/4]
   ...
   - [ ] Tool to make up a scenario
     - [ ] Choose file format
     - [ ] Manage to read Erlang term in file
     - [ ] Fancy options to control product creation, solution checking
   - [ ] Add functions instead of sending messages (hide the messages) for scenarios
