...
create(Module, Id, Opt, State, In, Out, InOut, Capacity, Work_time, List_data) ->
	...
	Actor = #config{
		module    = Module, 
		id        = Id, 
		opt       = ets:new(list_to_atom(lists:concat(["options_table_", Module,Id])), [duplicate_bag, public]), 
		state     = State, 
		in        = In, 
		out       = Out, 
		in_out    = InOut, 
		capacity  = Capacity, 
		work_time = Work_time, 
		list_data = ets:new(list_to_atom(lists:concat(["list_data_",Module, Id])), [ordered_set, public])},
	Actor1     = add_options_helper(Actor, Opt),
	TableQueue = ets:new(list_to_atom(lists:concat(["internal_queue_",Module, Id])), [duplicate_bag, public]),
